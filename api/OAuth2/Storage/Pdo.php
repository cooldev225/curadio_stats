    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    /* Added by Khvicha - 20140727 */
    public function deleteOldTokens($lifetime)
    {
    	if($lifetime > 0) {
    		$stmt = $this->db->prepare(sprintf('DELETE FROM %s WHERE (now()-expires) >= %d', $this->config['access_token_table'], $lifetime));
    		return $stmt->execute(compact('code'));
    	}
    	else {
    		return false;
