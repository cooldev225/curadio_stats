 ...
 * @param int $ot_sim (Optional, Similarity min range for OTHER music, default: 93)
 * @param int $FPEQ (Optional, pass 1 to enable FPEQ, default: 0)
 * @param string $return_fp (Optional, pass 'longFP' to return long/all footprint code for each similar block, pass 'shortFP' to return short footprint code withount notes and midsp for each similar block, default is empty string)
 * @param string $usr (Optional, pass 'cugate' to get CUGATE Track IDs in the result, default: '')
 * @return array
 */
function cug_fp_get_similars_from_tony_multirequest($fp_arrays, $en_sim=90, $cl_sim=93, $ot_sim=93, $FPEQ=0, $return_fp="", $usr="") {
	$result = array();
	...
	if(count($fp_arrays) > 0) {
		...
		foreach($fp_arrays as $fp_arr) {
			...
			$url .= "&matchType=en$".$en_sim."|cl$".$cl_sim."|ot$".$ot_sim;
			$url .= ($FPEQ) ? "&type=fpeq" : "";
			$url .= ($return_fp == "longFP") ? "&resData=longFP" : (($return_fp == "shortFP") ? "&resData=shortFP" : "");
			$url .= ($usr) ? "&usr=$usr" : "";
 ...
 * @return array
 */
function cug_fp_get_identical_track_from_similars_tony($similar_blocks_arr) {
	$result = array();
	...
	foreach($arr as $key=>$similar_blocks) {
		if(count($similar_blocks) > 0) {
			foreach($similar_blocks as $key2 => $similar_block) {
				...
				$percetanges = array();
				if(!empty($similar_block['slId']) && !empty($similar_block['similarity']) && !in_array($similar_block['slId'], $ids_arr_tmp)) {
					$ids_arr_tmp[] = $similar_block['slId'];
					$percetanges = cug_fp_choose_percentages_from_similar_blocks_by_track_id_tony($arr, $similar_block['slId'], $except_key=$key);
					//print_r($percetanges);
 ...
 * @return array
 */
function cug_fp_choose_percentages_from_similar_blocks_by_track_id_tony($arr, $track_id) {
	$result = array();

	foreach($arr as $key => $similar_blocks) {
		if(count($similar_blocks) > 0) {
			foreach($similar_blocks as $key2 => $similar_block) {
				if(!empty($similar_block['slId']) && !empty($similar_block['similarity']) && $similar_block['slId'] == $track_id) {
 ...
 * @return array
 */
function cug_fp_collect_similars_tony($similars_arr) {
	$result = array();

	$unique_ids = cug_fp_choose_unique_track_ids_from_similars_tony($similars_arr);
	//print_r($unique_ids);
	$arr = cug_fp_calculate_summarized_percentages_tony($similars_arr, $unique_ids);
	//print_r($arr);
 ...
 * @return array
 */
function cug_fp_choose_unique_track_ids_from_similars_tony($similars_arr) {
	$result = array();
	$tmp_arr = array();
	$tmp_index = 0;
	...
	foreach($similars_arr as $similar_blocks) {
		foreach($similar_blocks as $block) {
			if(!empty($block['slId']) && !empty($block['mediaName']) && !empty($block['mediaArtist'])) {
    		    $track_id = $block['slId'];
 ...
 * @return array
 */
function cug_fp_calculate_summarized_percentages_tony($similars_arr, $unique_ids) {
	$result = array();
	$index = 0;

	foreach($unique_ids as $track_id) {
		$result[$index]['slId'] = $track_id;
		$result[$index]['similarity_sum'] = 0;
		$result[$index]['blocks_num'] = 0;

		foreach($similars_arr as $similar_blocks) {
			foreach($similar_blocks as $block) {
				if(!empty($block['slId']) && !empty($block['similarity'])) {
    			    if($block['slId'] == $track_id) {
    					$result[$index]['similarity_sum'] += $block['similarity'];
    					$result[$index]['blocks_num'] += 1;
    					
    					$result[$index]['fp'] = !empty($block['fp']) ? $block['fp'] : "";
    				}
				}
			}
		}
 ...
 * @return array
 */
function cug_fp_parse_track_info_tony($arr) {
	$result = array();
	
	if(count($arr) > 0) {
		$shenzhen_id = !empty($arr['slId']) ? $arr['slId'] : "";
...
/**
 * Get Mood by KEY Value
 * 
 * @param int $key_val (KEY value from Footprint)
 * @param string $mode (optional, 'title' - return mood title; 'id' - return mood id; 'both' - return array('id', 'title'); default is 'title')
 * @return mixed (string|int|array)
 */
function cug_fp_get_mood_by_key($key_val, $mode="title") {
    global $mysqli, $Tables;
    $result = "";
    
    if($key_val >= 0) {
        $query = "SELECT mkr.mood_id AS id, m.title FROM {$Tables['mood_key_rel']} AS mkr ";
        $query .= "LEFT JOIN {$Tables['mood']} AS m ON m.id=mkr.mood_id ";
        $query .= "WHERE mkr.key_val=$key_val";
